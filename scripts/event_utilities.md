...
This script provides utility functions for managing and interacting with in-game events, particularly for streaming integration. It includes functions for player interaction, entity manipulation, and event selection/management.

## Player Utilities

These functions provide easy access to player-related information.

### Key Functions

*   `get_player()`: Returns the first player entity found with the "player_unit" tag.
*   `get_players()`: Returns a list of all player entities with the "player_unit" tag.
*   `get_player_position()`: Returns the transform (position) of the primary player.
*   `get_player_with_position()`: Returns the primary player entity along with its X and Y coordinates.
*   `get_enemies_in_radius(radius)`: Returns a list of enemy entities within a specified radius of the player.

## Entity Visuals

Functions for adding visual elements to entities, such as icons and text.

### Key Functions

*   `add_icon_above_head(game_effect_entity, icon_file, event, custom_name)`: Adds a UI icon to be displayed above an entity's head.
    *   `game_effect_entity`: The entity to attach the icon to.
    *   `icon_file`: The path to the sprite file for the icon.
    *   `event`: The event object, used for `ui_name` if `custom_name` is not provided.
    *   `custom_name`: An optional custom name for the icon.
*   `add_icon_in_hud(game_effect_entity, icon_file, event, custom_name)`: Adds a UI icon to be displayed in the HUD.
    *   Parameters are similar to `add_icon_above_head`, with `display_above_head` set to `false` and `display_in_hud` set to `true`.
*   `add_text_above_head(entity_id, text)`: Creates and attaches a text sprite above an entity.
    *   `entity_id`: The ID of the entity to attach the text to.
    *   `text`: The string content to display.
*   `add_timer_above_head(entity_id, event_id, event_timer)`: Creates and attaches a timer sprite above an entity. This timer displays remaining time for an event.
    *   `entity_id`: The ID of the entity to attach the timer to.
    *   `event_id`: The ID of the event the timer is associated with.
    *   `event_timer`: The duration of the timer in frames.

## Event Management

Functions related to the selection, weighting, and activation of streaming events.

### Key Functions

*   `_reflect()`: Registers all defined `streaming_events` for use in the game's options menu.
*   `_streaming_on_vote_start()`: Initializes event weights and decrements cooldowns at the start of a voting cycle.
*   `_streaming_get_event_for_vote()`: Selects a random event based on its weight and cooldown status. If no events are available, it attempts to select one ignoring cooldowns, and as a last resort, returns a default event.
    *   **Event Selection Logic**: Events are weighted, and a random number is generated against the total weight. Events with active cooldowns are excluded unless no other events are available.
    *   **Weight Adjustment**: The weight of a selected event is dynamically adjusted to influence future selections.
    *   **Cooldown**: Selected events are placed on a cooldown for a set duration.
*   `_streaming_run_event(id)`: Executes the action associated with a given event ID. This can involve delayed actions (using timers) or immediate actions.
*   `_streaming_set_event_enabled(id, enabled)`: Toggles the enabled state of a specific streaming event.

### Event Data Structure (Implicit)

While not explicitly defined in this snippet, the `streaming_events` table (referenced in `_reflect` and `_streaming_get_event_for_vote`) is expected to contain entries with at least the following properties:

| Property          | Type    | Description                                                                 |
| :---------------- | :------ | :-------------------------------------------------------------------------- |
| `id`              | string  | Unique identifier for the event.                                            |
| `ui_name`         | string  | Display name for the event in the UI.                                       |
| `ui_description`  | string  | Description of the event for the UI.                                        |
| `ui_icon`         | string  | Path to the UI icon sprite for the event.                                   |
| `ui_author`       | string  | Author of the event.                                                        |
| `kind`            | string  | Category or type of the event.                                              |
| `weight`          | number  | Base weight influencing the probability of selection (default: 1.0).        |
| `action`          | function| Function to execute when the event is run.                                  |
| `action_delayed`  | boolean | If true, the action is delayed (e.g., via timer).                           |
| `delay_timer`     | number  | Duration of the delay timer in frames if `action_delayed` is true.          |
| `cooldown`        | number  | Internal cooldown counter for the event.                                    |
| `enabled`         | boolean | Internal flag to track if the event is currently enabled.                   |

## Other Utilities

*   `set_lifetime(entity_id, multiplier_)`: Adjusts the lifetime of an entity by multiplying its current lifetime.
*   `get_lifetime()`: Retrieves the current voting cycle duration in frames, used as a base for entity lifetimes.
